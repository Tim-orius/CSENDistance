...
##Use this file for feature extraction for the Xception or InceptionV3 model
argparser.add_argument('-m', '--model', help='Model name: Xception or InceptionV3')
...
# Load the model.
#uncomment for the inveptionV3 model as it needs to be lower cased but other models not
##modelName = 'inception_v3'
function_name = 'tf.keras.applications.' + modelName
model = eval(function_name + "(include_top=False, weights='imagenet', input_shape=(128, 128, 3), pooling='max')")
...
for idx, row in df_pairs.iterrows():
	...
	##adjusted the size here too. matches the 128 above
	Object0 = cv2.resize(im[y01:y02, x01:x02, :], (128,128))
	Object1 = cv2.resize(im[y11:y12, x11:x12, :], (128, 128))
	...
	#uncoment the below line again for the inception model
	##modelName = 'inception_v3'
	function_name = 'tf.keras.applications.' + modelName + '.preprocess_input'
