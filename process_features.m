...
fprintf('size(objectFeatures) is [%s]\n', int2str(size(objectFeatures)))
%B = reshape(objectFeatures,[],1,512);
%fprintf('size(objectFeatures[0]) is [%s]\n', int2str(size(B)))
angles = gtd(:, 1);
meters = gtd(:, 3);
%fprintf('angles is [%s]\n', int2str(angles))
%fprintf('meter [%s]\n', int2str(meters))
...
%fprintf('codebook [%s]\n', int2str(codebook))
%fprintf('meters_quant [%s]\n', int2str(meters_quant))

objectFeatures(meters_quant == -1, :) = [];
%B(meters_quant == -1, :) = [];
%fprintf('object features [%s]\n', int2str(size(objectFeatures)))
meters(meters_quant == -1, :) = [];
meters_quant(meters_quant == -1, :) = [];
%fprintf('meters [%s]\n', int2str(length(meters)))
%fprintf('meters_quant [%s]\n', int2str(length(meters_quant)))
...
%fprintf('label) is [%s]\n', int2str(label))
reallabel = meters;
%fprintf('reallabel is [%s]\n', int2str(reallabel))
%fprintf('label length [%s]\n' , int2str(length(label)))
%fprintf('meters length [%s]\n' , int2str(length(meters)))


%adjust the dictionary size depending on which model is used! 
%VGG19 = 15;DenseNet121 = 9;ResNet50 = 13; Xception and InceptionV3 = 12
